<template>
  <div>
    ...
    <p class="comment">
      ...
      <code>--</code> (double dash) denoting modifiers to classes and
      <code>__</code> (double underline) denoting elements in a block. For example:
    </p>
    <pre>
    ...
    <p class="comment">
      For
      <code>scss</code> it means that we should use nesting of classes whenever possible:
    </p>
    <pre>
...
.block-element {
  ...
  computed: {
    highlightedExample() {
      return hljs.highlight("html", this.example).value;
    },
    highlightedExampleCss() {
      return hljs.highlight("scss", this.exampleCss).value;
    }
  }
};
</script>
